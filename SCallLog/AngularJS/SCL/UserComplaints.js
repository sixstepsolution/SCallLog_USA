app.controller('DashBoardCtrl', function ($scope, $http, $localStorage) {
    ...
    $http.get("../SCall/getUserComplaintsCount").then(
        function (response) {
            if (response.data.success == true) {
                console.log(response.data);
                var i = 0;
                $scope.TotalCounts = response.data.UsersCount;
                $scope.TotalCounts.forEach(function (a) {
                    $scope.UsersCounts.push({
                ...
                $scope.compUserLocations = response.data.CompLocations;
    ...
    $scope.showDepartment = function (usrID) {
        $scope.DepartmentCounts = [];
        $scope.selectedUserID = usrID;
        $scope.isUsers = false;
        $scope.isDeaprtments = true;
        $scope.isCategory = false;
        $scope.isSubCategory = false;
        $http.get("../SCall/getComplaintUsersCount?userID=" + usrID).then(
            function (response) {
                if (response.data.success == true) {
                    console.log(response.data);
                    var i = 0;
    ...
    $scope.showCategory = function (department, DeptID, color) {
        ...
        $http.get("../SCall/getComplaintsCategorywiseUserCount?Department=" + department + "&DeptID=" + DeptID + "&userID=" + $scope.selectedUserID).then(
            function (response) {
                if (response.data.success == true) {
                    console.log(response.data);
    ...
    $scope.showSubCategory = function (Category, CatID) {
        ...
        $http.get("../SCall/getComplaintsSubCategorywiseUsersCount?Category=" + Category + "&CatID=" + CatID + "&userID=" + $scope.selectedUserID).then(
            function (response) {
                if (response.data.success == true) {
                    console.log(response.data);
    ...
    $scope.viewDashBoard = function () {
        ...
        if ($scope.searchColumn == 'User') {
            ...
            $scope.showLeafletMap($scope.compUserLocations);

        } else if ($scope.searchColumn == 'Department') {
            $scope.isUsers = false;
            $scope.isDeaprtments = true;
            $scope.isCategory = false;
            $scope.isSubCategory = false;
    ...
    $scope.getAllUserDepartmentComplaints = function (Department, Search) {
        //alert(Search)
        ...
        $http.get("../SCall/getAllUserDepartmentComplaints?pageIndex=" + $scope.pageIndex
