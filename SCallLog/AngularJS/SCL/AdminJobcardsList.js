app.controller('AdminJobcardcntrl', function ($scope, $http, $filter) {
    ...
    $scope.getComplaintList = function () {
        $scope.complaints = [];
        $http.get("../User/getJobCardsByCompany?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&sorting=" + $scope.sorting + "&search=" + $scope.search).then(
            function (response) {
    ...
    //Map functionality
    $scope.viewJobCardMapDetails = function (complaintId) {
        //console.log("mapData");
        //console.log(mapData);
        $scope.LoadComplaintsOnMap(complaintId);
        //alert();
