app.controller('ComplaintCTRL', function ($scope, $http, $filter) {
    ...
    $scope.getComplaintList = function () {
        $scope.complaints = [];
        $http.get("../User/getAssignedComplaints?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&sorting=" + $scope.sorting + "&search=" + $scope.search).then(
            function (response) {
    ...
    //            "&EDate=" + compData.EndDate + "&SHours=" + compData.ServiceHours + "&Comments=" + compData.Comments + "&VehicalNo=" + compData.VehicalNo).then(
    //            function (response) {
    //                if (response.data.success) {
    ...
    $scope.ComplaintStatusChange = function (compData) {
        compData.ServiceHours = "";
        //alert($("#SDate").val());
        compData.PStatus = $scope.PStatus;
        compData.ID = $scope.CompUpdateID;
        ...
        if (compData == undefined || compData == "") {
        ...
        else if (compData.StartDate == undefined || compData.StartDate == "") {
            alertify.alert('Error', "Please select start date", function () {
            });
        }
        else if (compData.EndDate == undefined || compData.EndDate == "") {
            alertify.alert('Error', "Please select end date", function () {
            });
        }
        else if (compData.Comments == undefined || compData.Comments == "") {
            alertify.alert('Error', "Please Enter Comments", function () {
            ...
            $http.post("../User/updateJobcardStatus", { jobData: compData}).then(
                    function (response) {
                        if (response.data.success) {
                            delete $scope.CompUpdate;
                            $('#myModal').modal('hide');
